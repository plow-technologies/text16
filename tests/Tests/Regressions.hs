...
import qualified Data.Text16.Lazy.Encoding as LE
...
-- string (via either Array.new or Text16.replicate) could result in an
-- integer overflow.
replicate_crash :: IO ()
...
t301 = do
    ...
    assertEqual "The new array still contains the original value"
                (T.Text16 newArr originalOff originalLen)
                original
  where
    original@(T.Text16 originalArr originalOff originalLen) = T.pack "1234567890"
    T.Text16 newArr _off _len = T.take 1 $ T.drop 1 original
