-- | Benchmarks various pure functions from the Text16 library
--
    ...
    bgroup "Pure"
        ...
        , bgroup "Builder"
            [ bench ("mappend char+" ++ kind) $
                nf (TL.length . TB.toLazyText16 . mappendNChar 'a') 10000
            , bench ("mappend 8 char+" ++ kind) $
                nf (TL.length . TB.toLazyText16 . mappend8Char) 'a'
            , bench ("mappend text+" ++ kind) $
                nf (TL.length . TB.toLazyText16 . mappendNText16 short) 10000
            ]
        ]
  where
    benchT   = bench ("Text16+" ++ kind)
    benchTL  = bench ("LazyText16+" ++ kind)
...
--
mappendNText16 :: T.Text16 -> Int -> TB.Builder
mappendNText16 t n = go 0
  where
    go i
      | i < n     = TB.fromText16 t `mappend` go (i+1)
      | otherwise = mempty
