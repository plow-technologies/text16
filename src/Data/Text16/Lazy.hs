...
-- have better time complexity than their "Data.Text16" equivalents, due
-- to the underlying representation being a list of chunks. For other
-- operations, lazy 'Text16's are usually within a few percent of strict
...
import qualified Data.Text16.Unsafe as T
import qualified Data.Text16.Internal.Lazy.Fusion as S
import Data.Text16.Internal.Fusion.Types (PairS(..))
import Data.Text16.Internal.Lazy.Fusion (stream, unstream)
import Data.Text16.Internal.Lazy (Text16(..), chunk, empty, foldlChunks,
                                foldrChunks, smallChunkSize)
import Data.Text16.Internal (firstf, safe, text)
import Data.Text16.Lazy.Encoding (decodeUtf8', encodeUtf8)
import qualified Data.Text16.Internal.Functions as F
import Data.Text16.Internal.Lazy.Search (indices)
...
--
-- > import Data.Text16.Lazy as T
-- > import Data.Text16.Lazy.Encoding as E
...
-- allocate one 'ByteString' value, and two 'Text16' values. However,
-- when a module is compiled with optimisation enabled under GHC, the
-- two intermediate 'Text16' values will be optimised away, and the
...
compareText16 Empty Empty = EQ
compareText16 Empty _     = LT
compareText16 _     Empty = GT
compareText16 (Chunk a0 as) (Chunk b0 bs) = outer a0 b0
 where
  outer ta@(T.Text16 arrA offA lenA) tb@(T.Text16 arrB offB lenB) = go 0 0
   where
    go !i !j
      | i >= lenA = compareText16 as (chunk (T.Text16 arrB (offB+j) (lenB-j)) bs)
      | j >= lenB = compareText16 (chunk (T.Text16 arrA (offA+i) (lenA-i)) as) bs
      | a < b     = LT
      | a > b     = GT
...
-- @"Data.Text16".@'Data.Text16.Text16'
instance Data Text16 where
  gfoldl f z txt = z pack `f` (unpack txt)
  toConstr _     = packConstr
  gunfold k z c  = case constrIndex c of
    1 -> k (z pack)
    _ -> error "Data.Text16.Lazy.Text16.gunfold"
...
textDataType = mkDataType "Data.Text16.Lazy.Text16" [packConstr]

-- | /O(n)/ Convert a 'String' into a 'Text16'.
--
...
-- | /O(n)/ Convert a 'Text16' into a 'String'.
-- Subject to fusion.
unpack ::
#if defined(ASSERTS)
  HasCallStack =>
...
-- | /O(c)/ Convert a list of strict 'T.Text16's into a lazy 'Text16'.
fromChunks :: [T.Text16] -> Text16
fromChunks cs = L.foldr chunk Empty cs

-- | /O(n)/ Convert a lazy 'Text16' into a list of strict 'T.Text16's.
toChunks :: Text16 -> [T.Text16]
toChunks cs = foldrChunks (:) [] cs

-- | /O(n)/ Convert a lazy 'Text16' into a strict 'T.Text16'.
toStrict :: Text16 -> T.Text16
toStrict t = T.concat (toChunks t)
{-# INLINE [1] toStrict #-}

-- | /O(c)/ Convert a strict 'T.Text16' into a lazy 'Text16'.
fromStrict :: T.Text16 -> Text16
...
-- | /O(1)/ Adds a character to the front of a 'Text16'.  Subject to fusion.
...
-- | /O(n\/c)/ Appends one 'Text16' to another.  Subject to fusion.
...
-- | /O(n\/c)/ Returns all but the last character of a 'Text16', which must
...
-- | /O(n\/c)/ Returns the 'init' and 'last' of a 'Text16', or 'Nothing' if
-- empty.
--
...
-- | /O(n\/c)/ Returns the last character of a 'Text16', which must be
-- non-empty.  Subject to fusion.
last :: Text16 -> Char
...
compareLength :: Text16 -> Int64 -> Ordering
...
-- between the characters of a 'Text16'.  Subject to fusion.  Performs
...
justifyLeft :: Int64 -> Char -> Text16 -> Text16
justifyLeft k c t
    | len >= k  = t
...
justifyRight :: Int64 -> Char -> Text16 -> Text16
justifyRight k c t
    | len >= k  = t
...
center :: Int64 -> Char -> Text16 -> Text16
center k c t
    | len >= k  = t
...
-- 'Text16'.  Performs replacement on invalid scalar values.
...
-- | @'repeat' x@ is an infinite 'Text16', with @x@ the value of every
-- element.
--
-- @since 1.2.0.5
repeat :: Char -> Text16
...
-- | 'cycle' ties a finite, non-empty 'Text16' into a circular one, or
-- equivalently, the infinite repetition of the original 'Text16'.
--
-- @since 1.2.0.5
cycle :: Text16 -> Text16
...
-- | @'iterate' f x@ returns an infinite 'Text16' of repeated applications
-- of @f@ to @x@:
--
-- > iterate f x == [x, f x, f (f x), ...]
--
-- @since 1.2.0.5
iterate :: (Char -> Char) -> Char -> Text16
...
replicateChar :: Int64 -> Char -> Text16
...
-- Subject to fusion.
-- Performs replacement on invalid scalar values.
unfoldrN :: Int64 -> (a -> Maybe (Char,a)) -> a -> Text16
...
--
-- @since 1.1.1.0
takeEnd :: Int64 -> Text16 -> Text16
takeEnd n t0
    | n <= 0    = empty
    | otherwise = takeChunk n empty . L.reverse . toChunks $ t0
  where
    takeChunk :: Int64 -> Text16 -> [T.Text16] -> Text16
    takeChunk _ acc [] = acc
...
--
-- @since 1.1.1.0
dropEnd :: Int64 -> Text16 -> Text16
dropEnd n t0
    | n <= 0    = t0
    | otherwise = dropChunk n . L.reverse . toChunks $ t0
  where
    dropChunk :: Int64 -> [T.Text16] -> Text16
    dropChunk _ [] = empty
    dropChunk m (t:ts)
...
-- values dropped, or the empty 'Text16' if @n@ is greater than the
-- number of 'Word16' values present.
dropWords :: Int64 -> Text16 -> Text16
dropWords i t0
    | i <= 0    = t0
    | otherwise = drop' i t0
  where
    ...
    drop' 0 ts           = ts
    drop' _ Empty        = Empty
    drop' n (Chunk (T.Text16 arr off len) ts)
...
splitAt :: Int64 -> Text16 -> (Text16, Text16)
splitAt = loop
  where
    loop :: Int64 -> Text16 -> (Text16, Text16)
...
splitAtWord :: Int64 -> Text16 -> PairS Text16 Text16
splitAtWord _ Empty = empty :*: empty
splitAtWord x (Chunk c@(T.Text16 arr off len) cs)
...
-- | The 'group' function takes a 'Text16' and returns a list of 'Text16's
...
-- | /O(n)/ Return all initial segments of the given 'Text16',
-- shortest first.
inits :: Text16 -> [Text16]
inits = (Empty :) . inits'
  where inits' Empty        = []
...
splitOn pat src
  ...
  where
    ...
    l = foldlChunks (\a (T.Text16 _ _ b) -> a + intToInt64 b) 0 pat
{-# INLINE [1] splitOn #-}
...
-- | /O(n)/ 'Text16' index (subscript) operator, starting from 0.
-- Subject to fusion.
index :: Text16 -> Int64 -> Char
...
revChunks :: [T.Text16] -> Text16
revChunks = L.foldl' (flip chunk) Empty

emptyError :: String -> a
emptyError fun = P.error ("Data.Text16.Lazy." ++ fun ++ ": empty input")

impossibleError :: String -> a
impossibleError fun = P.error ("Data.Text16.Lazy." ++ fun ++ ": impossible case")
