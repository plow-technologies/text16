...
import Data.Text16.Internal.Fusion (stream)
import Data.Text16.Internal.Fusion.Types (Step(..), Stream(..))
import Data.Text16.Internal.IO (hGetLineWith, readChunk)
...
readChunkEof :: Handle__ -> CharBuffer -> IO (Text16, Bool)
...
hGetLine :: Handle -> IO Text16
hGetLine = hGetLineWith T.concat

-- | Write a string to a handle.
hPutStr :: Handle -> Text16 -> IO ()
-- This function is lifted almost verbatim from GHC.IO.Handle.Text16.
hPutStr h t = do
  (buffer_mode, nl) <-
...
-- The following functions are largely lifted from GHC.IO.Handle.Text16,
...
-- This function is completely lifted from GHC.IO.Handle.Text16.
...
-- > import Data.Text16 (Text16)
-- > import Data.Text16.Encoding (encodeUtf16)
-- >
-- > putStr_Utf16LE :: Text16 -> IO ()
-- > putStr_Utf16LE t = B.putStr (encodeUtf16LE t)
--
-- On transcoding errors, an 'IOError' exception is thrown. You can
-- use the API in "Data.Text16.Encoding" if you need more control over
