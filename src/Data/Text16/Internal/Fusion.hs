...
-- |
-- Module      : Data.Text16.Internal.Fusion
...
--
-- Text16 manipulation functions represented as fusible operations over
-- streams.
module Data.Text16.Internal.Fusion
    (
    -- * Types
...
import Data.Text16.Internal.Unsafe.Char (ord, unsafeChr, unsafeWrite)
import Data.Text16.Internal.Unsafe.Shift (shiftL, shiftR)
import qualified Data.Text16.Array as A
import qualified Data.Text16.Internal.Fusion.Common as S
import Data.Text16.Internal.Fusion.Types
import Data.Text16.Internal.Fusion.Size
import qualified Data.Text16.Internal as I
...
-- | /O(n)/ Convert a 'Text16' into a 'Stream Char'.
stream ::
#if defined(ASSERTS)
  HasCallStack =>
#endif
  Text16 -> Stream Char
stream (Text16 arr off len) = Stream next off (betweenSize (len `shiftR` 1) len)
    where
      !end = off+len
      next !i
          | i >= end                   = Done
...
-- | /O(n)/ Convert a 'Text16' into a 'Stream Char', but iterate
-- backwards.
reverseStream :: Text16 -> Stream Char
reverseStream (Text16 arr off len) = Stream next (off+len-1) (betweenSize (len `shiftR` 1) len)
    where
      {-# INLINE next #-}
      next !i
          | i < off                    = Done
...
-- | /O(n)/ Convert a 'Stream Char' into a 'Text16'.
unstream :: Stream Char -> Text16
unstream (Stream next0 s0 len) = runText16 $ \done -> do
...
-- parameter from left to right, and returns a final 'Text16'.
mapAccumL ::
#if defined(ASSERTS)
  HasCallStack =>
#endif
  (a -> Char -> (a,Char)) -> a -> Stream Char -> (a, Text16)
