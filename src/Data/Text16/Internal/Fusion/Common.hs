...
-- |
-- Module      : Data.Text16.Internal.Fusion.Common
...
import Data.Text16.Internal.Fusion.CaseMapping (foldMapping, lowerMapping, titleMapping,
                                     upperMapping)
...
-- | /O(1)/ Returns the first character of a Text16, which must be non-empty.
...
-- | /O(n)/ all @p @xs determines if all characters in the 'Text16'
...
-- | /O(n)/ minimum returns the minimum value from a 'Text16', which must be
-- non-empty.
...
streamError func msg = P.error $ "Data.Text16.Internal.Fusion.Common." ++ func ++ ": " ++ msg
