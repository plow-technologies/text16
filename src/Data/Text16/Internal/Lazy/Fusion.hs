...
-- |
-- Module      : Data.Text16.Lazy.Fusion
...
module Data.Text16.Internal.Lazy.Fusion
    (
      stream
    , unstream
    , unstreamChunks
...
import Data.Text16.Internal.Fusion.Size (isEmpty, unknownSize)
import Data.Text16.Internal.Lazy
...
stream text = Stream next (text :*: 0) unknownSize
  where
    next (Empty :*: _) = Done
    next (txt@(Chunk t@(I.Text16 _ _ len) ts) :*: i)
        | i >= len  = next (ts :*: 0)
        | otherwise = Yield c (txt :*: i+d)
        where Iter c d = iter t i
{-# INLINE [0] stream #-}

-- | /O(n)/ Convert a 'Stream Char' into a 'Text16', using the given
-- chunk size.
unstreamChunks ::
#if defined(ASSERTS)
  HasCallStack =>
#endif
  Int -> Stream Char -> Text16
    ...
    outer so = {-# SCC "unstreamChunks/outer" #-}
      ...
      where
        ...
        finish marr len s' = do
          ...
          return (I.Text16 arr 0 len `Chunk` outer s')
{-# INLINE [0] unstreamChunks #-}

-- | /O(n)/ Convert a 'Stream Char' into a 'Text16', using
-- 'defaultChunkSize'.
unstream ::
#if defined(ASSERTS)
  HasCallStack =>
