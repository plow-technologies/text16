...
import Data.Text16.Internal.Unsafe (inlinePerformIO)
...
array_size_error :: a
array_size_error = error "Data.Text16.Array.new: size overflow"
...
unsafeIndex a@Array{..} i@(I# i#) =
  ...
  if i < 0 || i >= word16len
  then error ("Data.Text16.Array.unsafeIndex: bounds error, offset " ++ show i ++ ", length " ++ show word16len)
  else
#endif
...
unsafeWrite ma@MArray{..} i@(I# i#) (W16# e#) = ST $ \s1# ->
  ...
  if i < 0 || i >= word16len then error ("Data.Text16.Array.unsafeWrite: bounds error, offset " ++ show i ++ ", length " ++ show word16len) else
#endif
