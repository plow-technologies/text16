...
import Data.Text16.Internal.Fusion (stream, reverseStream, unstream)
import Data.Text16.Internal.Private (span_)
import Data.Text16.Internal (Text16(..), empty, firstf, mul, safe, text)
import Data.Text16.Show (singleton, unpack, unpackCString#)
import qualified Prelude as P
import Data.Text16.Unsafe (Iter(..), iter, iter_, lengthWord16, reverseIter,
                         reverseIter_, unsafeHead, unsafeTail)
import Data.Text16.Internal.Unsafe.Char (unsafeChr)
import qualified Data.Text16.Internal.Functions as F
import qualified Data.Text16.Internal.Encoding.Utf16 as U16
import Data.Text16.Internal.Search (indices)
import Data.Text16.Internal.Unsafe.Shift (UnsafeShift(..))
...
-- $strict
--
-- This package provides both strict and lazy 'Text16' types.  The
-- strict type is provided by the "Data.Text16" module, while the lazy
-- type is provided by the "Data.Text16.Lazy" module. Internally, the
-- lazy @Text16@ type consists of a list of strict chunks.
--
-- The strict 'Text16' type requires that an entire string fit into
-- memory at once.  The lazy 'Data.Text16.Lazy.Text16' type is capable of
...
-- the lazy 'Data.Text16.Lazy.Text16' type slower than its strict
...
-- $replacement
--
-- A 'Text16' value is a sequence of Unicode scalar values, as defined
-- in
-- <http://www.unicode.org/versions/Unicode5.2.0/ch03.pdf#page=35 §3.9, definition D76 of the Unicode 5.2 standard >.
-- As such, a 'Text16' cannot contain values in the range U+D800 to
...
-- those cases.
--
-- Within this module, many functions construct a 'Text16' from one or
...
-- 'Text16' value is represented as packed UTF-16 data. Values in the
...
-- most one 'Text16' value.
--
-- As an example, consider the following pipeline:
--
-- > import Data.Text16 as T
-- > import Data.Text16.Encoding as E
-- > import Data.ByteString (ByteString)
-- >
...
-- allocate one 'Data.ByteString.ByteString' value, and two 'Text16'
-- values. However, when a module is compiled with optimisation
-- enabled under GHC, the two intermediate 'Text16' values will be
...
instance Eq Text16 where
    Text16 arrA offA lenA == Text16 arrB offB lenB
        | lenA == lenB = A.equal arrA offA arrB offB lenA
        | otherwise    = False
    {-# INLINE (==) #-}

instance Ord Text16 where
    compare = compareText16

instance Read Text16 where
    readsPrec p str = [(pack x,y) | (x,y) <- readsPrec p str]

-- | @since 1.2.2.0
instance Semigroup Text16 where
    (<>) = append

instance Monoid Text16 where
    mempty  = empty
    mappend = (<>)
    mconcat = concat

instance IsString Text16 where
    fromString = pack

-- | @since 1.2.0.0
instance Exts.IsList Text16 where
    type Item Text16 = Char
    fromList       = pack
    toList         = unpack

instance NFData Text16 where rnf !_ = ()

-- | @since 1.2.1.0
instance Binary Text16 where
...
-- <https://mail.haskell.org/pipermail/haskell-cafe/2010-January/072379.html could we get a Data instance for Data.Text16.Text16? >
--
...
-- | @since 1.2.2.0
instance PrintfArg Text16 where
...
textDataType = mkDataType "Data.Text16.Text16" [packConstr]

-- | /O(n)/ Compare two 'Text16' values lexicographically.
compareText16 :: Text16 -> Text16 -> Ordering
compareText16 ta@(Text16 _arrA _offA lenA) tb@(Text16 _arrB _offB lenB)
    | lenA == 0 && lenB == 0 = EQ
    | otherwise              = go 0 0
  where
    go !i !j
...
-- -----------------------------------------------------------------------------
-- * Conversion to/from 'Text16'

-- | /O(n)/ Convert a 'String' into a 'Text16'.  Subject to
...
-- | /O(n)/ Adds a character to the front of a 'Text16'.  This function
-- is more costly than its 'List' counterpart because it requires
-- copying a new array.  Subject to fusion.  Performs replacement on
-- invalid scalar values.
cons :: Char -> Text16 -> Text16
cons c t = unstream (S.cons (safe c) (stream t))
{-# INLINE cons #-}

infixr 5 `cons`

-- | /O(n)/ Adds a character to the end of a 'Text16'.  This copies the
-- entire array in the process, unless fused.  Subject to fusion.
-- Performs replacement on invalid scalar values.
snoc :: Text16 -> Char -> Text16
snoc t c = unstream (S.snoc (stream t) (safe c))
{-# INLINE snoc #-}

-- | /O(n)/ Appends one 'Text16' to the other by copying both of them
-- into a new 'Text16'.  Subject to fusion.
append :: Text16 -> Text16 -> Text16
append a@(Text16 arr1 off1 len1) b@(Text16 arr2 off2 len2)
    | len1 == 0 = b
    | len2 == 0 = a
    | len > 0   = Text16 (A.run x) 0 len
...
-- | /O(1)/ Returns the first character of a 'Text16', which must be
-- non-empty.  Subject to fusion.
head :: Text16 -> Char
head t = S.head (stream t)
{-# INLINE head #-}

-- | /O(1)/ Returns the first character and rest of a 'Text16', or
-- 'Nothing' if empty. Subject to fusion.
uncons :: Text16 -> Maybe (Char, Text16)
uncons t@(Text16 arr off len)
    | len <= 0  = Nothing
...
-- | /O(1)/ Returns the last character of a 'Text16', which must be
-- non-empty.  Subject to fusion.
last :: Text16 -> Char
last (Text16 arr off len)
...
-- | /O(1)/ Returns all characters after the head of a 'Text16', which
-- must be non-empty.  Subject to fusion.
tail :: Text16 -> Text16
tail t@(Text16 arr off len)
...
-- | /O(1)/ Returns all but the last character of a 'Text16', which must
-- be non-empty.  Subject to fusion.
init :: Text16 -> Text16
init (Text16 arr off len) | len <= 0                   = emptyError "init"
...
-- 'Text16', or 'Nothing' if empty.
--
-- @since 1.2.3.0
unsnoc :: Text16 -> Maybe (Text16, Char)
unsnoc (Text16 arr off len)
    | len <= 0                 = Nothing
...
-- | /O(1)/ Tests whether a 'Text16' is empty or not.  Subject to
-- fusion.
null :: Text16 -> Bool
null (Text16 _arr _off len) =
#if defined(ASSERTS)
    assert (len >= 0) $
#endif
    len <= 0
...
-- | /O(1)/ Tests whether a 'Text16' contains exactly one character.
-- Subject to fusion.
isSingleton :: Text16 -> Bool
isSingleton = S.isSingleton . stream
{-# INLINE isSingleton #-}

-- | /O(n)/ Returns the number of characters in a 'Text16'.
-- Subject to fusion.
length ::
#if defined(ASSERTS)
  HasCallStack =>
...
-- | /O(n)/ Compare the count of characters in a 'Text16' to a number.
-- Subject to fusion.
--
-- This function gives the same answer as comparing against the result
-- of 'length', but can short circuit if the count of characters is
-- greater than the number, and hence be more efficient.
compareLength :: Text16 -> Int -> Ordering
...
-- -----------------------------------------------------------------------------
-- * Transformations
-- | /O(n)/ 'map' @f@ @t@ is the 'Text16' obtained by applying @f@ to
-- each element of @t@.
--
-- Example:
--
...
map :: (Char -> Char) -> Text16 -> Text16
map f t = unstream (S.map (safe . f) (stream t))
{-# INLINE [1] map #-}

-- | /O(n)/ The 'intercalate' function takes a 'Text16' and a list of
-- 'Text16's and concatenates the list after interspersing the first
...
intercalate :: Text16 -> [Text16] -> Text16
intercalate t = concat . (F.intersperse t)
{-# INLINE intercalate #-}

-- | /O(n)/ The 'intersperse' function takes a character and places it
-- between the characters of a 'Text16'.
--
-- Example:
--
...
--
-- Subject to fusion.  Performs replacement on invalid scalar values.
intersperse     :: Char -> Text16 -> Text16
...
replace :: Text16
        ...
        -> Text16
replace needle@(Text16 _      _      neeLen)
               (Text16 repArr repOff repLen)
      haystack@(Text16 hayArr hayOff hayLen)
...
-- $case
--
-- When case converting 'Text16' values, do not use combinators like
...
-- <http://hackage.haskell.org/package/text-icu-0.6.3.7/docs/Data-Text16-ICU.html#g:4 case mapping functions from the text-icu package >.
...
-- "foobar"
justifyLeft :: Int -> Char -> Text16 -> Text16
justifyLeft k c t
    | len >= k  = t
...
-- "foobar"
justifyRight :: Int -> Char -> Text16 -> Text16
justifyRight k c t
    | len >= k  = t
...
-- of its 'Text16' argument.  Note that this function uses 'pack',
...
-- >>> transpose ["blue","red"]
-- ["br","le","ud","e"]
transpose :: [Text16] -> [Text16]
transpose ts = P.map pack (L.transpose (P.map unpack ts))

-- -----------------------------------------------------------------------------
-- * Reducing 'Text16's (folds)

-- | /O(n)/ 'foldl', applied to a binary operator, a starting value
-- (typically the left-identity of the operator), and a 'Text16',
-- reduces the 'Text16' using the binary operator, from left to right.
-- Subject to fusion.
foldl :: (a -> Char -> a) -> a -> Text16 -> a
...
-- and thus must be applied to a non-empty 'Text16'.  Subject to fusion.
foldl1 :: (Char -> Char -> Char) -> Text16 -> Char
...
-- (typically the right-identity of the operator), and a 'Text16',
-- reduces the 'Text16' using the binary operator, from right to left.
-- Subject to fusion.
foldr :: (Char -> a -> a) -> a -> Text16 -> a
foldr f z t = S.foldr f z (stream t)
{-# INLINE foldr #-}

-- | /O(n)/ A variant of 'foldr' that has no starting value argument,
-- and thus must be applied to a non-empty 'Text16'.  Subject to
-- fusion.
foldr1 :: (Char -> Char -> Char) -> Text16 -> Char
...
-- | /O(n)/ Concatenate a list of 'Text16's.
concat :: [Text16] -> Text16
concat ts = case ts' of
              [] -> empty
              [t] -> t
              _ -> Text16 (A.run go) 0 len
  where
    ...
    go = do
      arr <- A.new len
      let step i (Text16 a o l) =
            let !j = i + l in A.copyI arr i a o j >> return j
      foldM step 0 ts' >> return arr

-- | /O(n)/ Map a function over a 'Text16' that results in a 'Text16', and
-- concatenate the results.
concatMap :: (Char -> Text16) -> Text16 -> Text16
concatMap f = concat . foldr ((:) . f) []
{-# INLINE concatMap #-}

-- | /O(n)/ 'any' @p@ @t@ determines whether any character in the
-- 'Text16' @t@ satisfies the predicate @p@. Subject to fusion.
any :: (Char -> Bool) -> Text16 -> Bool
any p t = S.any p (stream t)
{-# INLINE any #-}

-- | /O(n)/ 'all' @p@ @t@ determines whether all characters in the
-- 'Text16' @t@ satisfy the predicate @p@. Subject to fusion.
all :: (Char -> Bool) -> Text16 -> Bool
all p t = S.all p (stream t)
{-# INLINE all #-}

-- | /O(n)/ 'maximum' returns the maximum value from a 'Text16', which
-- must be non-empty. Subject to fusion.
maximum :: Text16 -> Char
maximum t = S.maximum (stream t)
{-# INLINE maximum #-}

-- | /O(n)/ 'minimum' returns the minimum value from a 'Text16', which
-- must be non-empty. Subject to fusion.
minimum :: Text16 -> Char
...
scanl :: (Char -> Char -> Char) -> Char -> Text16 -> Text16
...
scanl1 :: (Char -> Char -> Char) -> Text16 -> Text16
...
scanr :: (Char -> Char -> Char) -> Char -> Text16 -> Text16
...
-- value argument. Performs replacement on invalid scalar values.
scanr1 :: (Char -> Char -> Char) -> Text16 -> Text16
scanr1 f t | null t    = empty
           | otherwise = scanr f (last t) (init t)
{-# INLINE scanr1 #-}

-- | /O(n)/ Like a combination of 'map' and 'foldl''. Applies a
-- function to each element of a 'Text16', passing an accumulating
-- parameter from left to right, and returns a final 'Text16'.  Performs
-- replacement on invalid scalar values.
mapAccumL :: (a -> Char -> (a,Char)) -> a -> Text16 -> (a, Text16)
...
-- 'Text16', passing an accumulating parameter from right to left, and
-- returning a final value of this accumulator together with the new
-- 'Text16'.
-- Performs replacement on invalid scalar values.
mapAccumR :: (a -> Char -> (a,Char)) -> a -> Text16 -> (a, Text16)
mapAccumR f z0 = second reverse . S.mapAccumL g z0 . reverseStream
    where g a b = second safe (f a b)
{-# INLINE mapAccumR #-}

-- -----------------------------------------------------------------------------
-- ** Generating and unfolding 'Text16's

-- | /O(n*m)/ 'replicate' @n@ @t@ is a 'Text16' consisting of the input
-- @t@ repeated @n@ times.
replicate :: Int -> Text16 -> Text16
replicate n t@(Text16 a o l)
    | n <= 0 || l <= 0       = empty
    | n == 1                 = t
    | isSingleton t          = replicateChar n (unsafeHead t)
    | otherwise              = Text16 (A.run x) 0 len
  where
...
-- | /O(n)/ 'replicateChar' @n@ @c@ is a 'Text16' of length @n@ with @c@ the
-- value of every element. Subject to fusion.
replicateChar :: Int -> Char -> Text16
replicateChar n c = unstream (S.replicateCharI n (safe c))
{-# INLINE replicateChar #-}

-- | /O(n)/, where @n@ is the length of the result. The 'unfoldr'
-- function is analogous to the List 'L.unfoldr'. 'unfoldr' builds a
-- 'Text16' from a seed value. The function takes the element and
-- returns 'Nothing' if it is done producing the 'Text16', otherwise
...
-- | /O(n)/ 'take' @n@, applied to a 'Text16', returns the prefix of the
-- 'Text16' of length @n@, or the 'Text16' itself if @n@ is greater than
-- the length of the Text16. Subject to fusion.
take :: Int -> Text16 -> Text16
take n t@(Text16 arr off len)
    | n <= 0    = empty
    | n >= len  = t
    | otherwise = text arr off (iterN n t)
{-# INLINE [1] take #-}

iterN :: Int -> Text16 -> Int
iterN n t@(Text16 _arr _off len) = loop 0 0
  where loop !i !cnt
            | i >= len || cnt >= n = i
...
--
-- @since 1.1.1.0
takeEnd :: Int -> Text16 -> Text16
takeEnd n t@(Text16 arr off len)
    | n <= 0    = empty
    | n >= len  = t
    | otherwise = text arr (off+i) (len-i)
  where i = iterNEnd n t

iterNEnd :: Int -> Text16 -> Int
iterNEnd n t@(Text16 _arr _off len) = loop (len-1) n
  where loop i !m
          | m <= 0    = i+1
          | i <= 0    = 0
...
-- | /O(n)/ 'drop' @n@, applied to a 'Text16', returns the suffix of the
-- 'Text16' after the first @n@ characters, or the empty 'Text16' if @n@
-- is greater than the length of the 'Text16'. Subject to fusion.
drop :: Int -> Text16 -> Text16
drop n t@(Text16 arr off len)
    | n <= 0    = t
    | n >= len  = empty
    | otherwise = text arr (off+i) (len-i)
  where i = iterN n t
...
--
-- @since 1.1.1.0
dropEnd :: Int -> Text16 -> Text16
dropEnd n t@(Text16 arr off len)
    | n <= 0    = t
    | n >= len  = empty
    | otherwise = text arr off (iterNEnd n t)

-- | /O(n)/ 'takeWhile', applied to a predicate @p@ and a 'Text16',
-- returns the longest prefix (possibly empty) of elements that
-- satisfy @p@.  Subject to fusion.
takeWhile :: (Char -> Bool) -> Text16 -> Text16
takeWhile p t@(Text16 arr off len) = loop 0
...
-- | /O(n)/ 'takeWhileEnd', applied to a predicate @p@ and a 'Text16',
...
--
-- @since 1.2.2.0
takeWhileEnd :: (Char -> Bool) -> Text16 -> Text16
takeWhileEnd p t@(Text16 arr off len) = loop (len-1) len
...
dropWhile :: (Char -> Bool) -> Text16 -> Text16
dropWhile p t@(Text16 arr off len) = loop 0 0
  where loop !i !l | l >= len  = empty
                   | p c       = loop (i+d) (l+d)
                   | otherwise = Text16 arr (off+i) (len-l)
            where Iter c d     = iter t i
{-# INLINE [1] dropWhile #-}
...
-- >>> dropWhileEnd (=='.') "foo..."
-- "foo"
dropWhileEnd :: (Char -> Bool) -> Text16 -> Text16
dropWhileEnd p t@(Text16 arr off len) = loop (len-1) len
  where loop !i !l | l <= 0    = empty
                   | p c       = loop (i+d) (l+d)
                   | otherwise = Text16 arr off l
            where (c,d)        = reverseIter t i
...
dropAround :: (Char -> Bool) -> Text16 -> Text16
...
splitAt :: Int -> Text16 -> (Text16, Text16)
splitAt n t@(Text16 arr off len)
    | n <= 0    = (empty, t)
    | n >= len  = (t, empty)
...
-- >>> T.span (=='0') "000AB"
-- ("000","AB")
span :: (Char -> Bool) -> Text16 -> (Text16, Text16)
...
-- >>> T.break (=='c') "180cm"
-- ("180","cm")
break :: (Char -> Bool) -> Text16 -> (Text16, Text16)
break p = span (not . p)
{-# INLINE break #-}

-- | /O(n)/ Group characters in a string according to a predicate.
groupBy :: (Char -> Char -> Bool) -> Text16 -> [Text16]
groupBy p = loop
  where
    loop t@(Text16 arr off len)
        | null t    = []
        | otherwise = text arr off n : loop (text arr (off+n) (len-n))
        where Iter c d = iter t 0
              n     = d + findAIndexOrEnd (not . p c) (Text16 arr (off+d) (len-d))
...
findAIndexOrEnd :: (Char -> Bool) -> Text16 -> Int
findAIndexOrEnd q t@(Text16 _arr _off len) = go 0
...
-- | /O(n)/ Return all initial segments of the given 'Text16', shortest
-- first.
inits :: Text16 -> [Text16]
inits t@(Text16 arr off len) = loop 0
    where loop i | i >= len = [t]
                 | otherwise = Text16 arr off i : loop (i + iter_ t i)

-- | /O(n)/ Return all final segments of the given 'Text16', longest
-- first.
tails :: Text16 -> [Text16]
...
-- copies to create substrings; they just construct new 'Text16's that
-- are slices of the original.

-- | /O(m+n)/ Break a 'Text16' into pieces separated by the first 'Text16'
...
splitOn :: Text16
        ...
        -- ^ Input text.
        -> [Text16]
splitOn pat@(Text16 _ _ l) src@(Text16 arr off len)
...
-- | /O(n)/ Splits a 'Text16' into components delimited by separators,
...
-- >>> split (=='a') ""
-- [""]
split :: (Char -> Bool) -> Text16 -> [Text16]
split _ t@(Text16 _off _arr 0) = [t]
split p t = loop t
...
-- | /O(n)/ Splits a 'Text16' into components of length @k@.  The last
...
-- | /O(n)/ The 'elem' function takes a character and a 'Text16', and
-- returns 'True' if the element is found in the given 'Text16', or
-- 'False' otherwise.
elem :: Char -> Text16 -> Bool
elem c t = S.any (== c) (stream t)
{-# INLINE elem #-}

-- | /O(n)/ The 'find' function takes a predicate and a 'Text16', and
-- returns the first element matching the predicate, or 'Nothing' if
-- there is no such element. Subject to fusion.
find :: (Char -> Bool) -> Text16 -> Maybe Char
find p t = S.findBy p (stream t)
{-# INLINE find #-}

-- | /O(n)/ The 'partition' function takes a predicate and a 'Text16',
-- and returns the pair of 'Text16's with elements which do and do not
-- satisfy the predicate, respectively; i.e.
--
-- > partition p t == (filter p t, filter (not . p) t)
partition :: (Char -> Bool) -> Text16 -> (Text16, Text16)
partition p t = (filter p t, filter (not . p) t)
{-# INLINE partition #-}

-- | /O(n)/ 'filter', applied to a predicate and a 'Text16',
-- returns a 'Text16' containing those characters that satisfy the
-- predicate.
filter :: (Char -> Bool) -> Text16 -> Text16
...
-- In (unlikely) bad cases, this function's time complexity degrades
-- towards /O(n*m)/.
breakOn :: Text16 -> Text16 -> (Text16, Text16)
breakOn pat src@(Text16 arr off len)
...
-- >>> breakOnEnd "::" "a::b::c"
-- ("a::b::","c")
breakOnEnd :: Text16 -> Text16 -> (Text16, Text16)
...
breakOnAll :: Text16              -- ^ @needle@ to search for
           -> Text16              -- ^ @haystack@ in which to search
           -> [(Text16, Text16)]
breakOnAll pat src@(Text16 arr off slen)
...
-- $index
--
-- If you think of a 'Text16' value as an array of 'Char' values (which
...
-- or trim the searched string.  With a 'Text16' value, this approach
...
-- | /O(n)/ 'Text16' index (subscript) operator, starting from 0. Subject to fusion.
index :: Text16 -> Int -> Char
index t n = S.index (stream t) n
{-# INLINE index #-}

-- | /O(n)/ The 'findIndex' function takes a predicate and a 'Text16'
-- and returns the index of the first element in the 'Text16' satisfying
-- the predicate. Subject to fusion.
findIndex :: (Char -> Bool) -> Text16 -> Maybe Int
findIndex p t = S.findIndex p (stream t)
{-# INLINE findIndex #-}

-- | /O(n+m)/ The 'count' function returns the number of times the
-- query string appears in the given 'Text16'. An empty query string is
-- invalid, and will cause an error to be raised.
--
-- In (unlikely) bad cases, this function's time complexity degrades
-- towards /O(n*m)/.
count :: Text16 -> Text16 -> Int
count pat src
...
-- query element appears in the given 'Text16'. Subject to fusion.
countChar :: Char -> Text16 -> Int
...
-- | /O(n)/ 'zip' takes two 'Text16's and returns a list of
-- corresponding pairs of bytes. If one input 'Text16' is short,
-- excess elements of the longer 'Text16' are discarded. This is
-- equivalent to a pair of 'unpack' operations.
zip :: Text16 -> Text16 -> [(Char,Char)]
...
-- Performs replacement on invalid scalar values.
zipWith :: (Char -> Char -> Char) -> Text16 -> Text16 -> Text16
zipWith f t1 t2 = unstream (S.zipWith g (stream t1) (stream t2))
    where g a b = safe (f a b)
{-# INLINE zipWith #-}

-- | /O(n)/ Breaks a 'Text16' up into a list of words, delimited by 'Char's
-- representing white space.
words :: Text16 -> [Text16]
words t@(Text16 arr off len) = loop 0 0
  where
    loop !start !n
        | n >= len = if start == n
                     then []
                     else [Text16 arr (start+off) (n-start)]
        | isSpace c =
            if start == n
            then loop (start+1) (start+1)
            else Text16 arr (start+off) (n-start) : loop (n+d) (n+d)
        | otherwise = loop start (n+d)
        where Iter c d = iter t n
{-# INLINE words #-}

-- | /O(n)/ Breaks a 'Text16' up into a list of 'Text16's at
...
{-
-- | /O(n)/ Portably breaks a 'Text16' up into a list of 'Text16's at line
-- boundaries.
--
...
-- each.
unlines :: [Text16] -> Text16
unlines = concat . L.map (`snoc` '\n')
{-# INLINE unlines #-}

-- | /O(n)/ Joins words using single space characters.
unwords :: [Text16] -> Text16
unwords = intercalate (singleton ' ')
{-# INLINE unwords #-}

-- | /O(n)/ The 'isPrefixOf' function takes two 'Text16's and returns
-- 'True' iff the first is a prefix of the second.  Subject to fusion.
isPrefixOf :: Text16 -> Text16 -> Bool
isPrefixOf a@(Text16 _ _ alen) b@(Text16 _ _ blen) =
...
-- | /O(n)/ The 'isSuffixOf' function takes two 'Text16's and returns
-- 'True' iff the first is a suffix of the second.
isSuffixOf :: Text16 -> Text16 -> Bool
isSuffixOf a@(Text16 _aarr _aoff alen) b@(Text16 barr boff blen) =
    d >= 0 && a == b'
  where d              = blen - alen
        b' | d == 0    = b
           | otherwise = Text16 barr (boff+d) alen
{-# INLINE isSuffixOf #-}

-- | /O(n+m)/ The 'isInfixOf' function takes two 'Text16's and returns
...
-- > import Data.Text16 as T
-- >
-- > fnordLength :: Text16 -> Int
-- > fnordLength (stripPrefix "fnord" -> Just suf) = T.length suf
-- > fnordLength _                                 = -1
stripPrefix :: Text16 -> Text16 -> Maybe Text16
stripPrefix p@(Text16 _arr _off plen) t@(Text16 arr off len)
...
-- >>> commonPrefixes "" "baz"
-- Nothing
commonPrefixes :: Text16 -> Text16 -> Maybe (Text16,Text16,Text16)
commonPrefixes t0@(Text16 arr0 off0 len0) t1@(Text16 arr1 off1 len1) = go 0 0
  where
    go !i !j | i < len0 && j < len1 && a == b = go (i+d0) (j+d1)
             | i > 0     = Just (Text16 arr0 off0 i,
...
-- > import Data.Text16 as T
-- >
-- > quuxLength :: Text16 -> Int
-- > quuxLength (stripSuffix "quux" -> Just pre) = T.length pre
-- > quuxLength _                                = -1
stripSuffix :: Text16 -> Text16 -> Maybe Text16
stripSuffix p@(Text16 _arr _off plen) t@(Text16 arr off len)
...
emptyError :: String -> a
emptyError fun = P.error $ "Data.Text16." ++ fun ++ ": empty input"

overflowError :: String -> a
overflowError fun = P.error $ "Data.Text16." ++ fun ++ ": size overflow"
...
copy :: Text16 -> Text16
copy (Text16 arr off len) = Text16 (A.run go) 0 len
  where
    go :: ST s (A.MArray s)
    go = do
      marr <- A.new len
...
--       verify the doctests via `doctest -fobject-code Data/Text16.hs`
